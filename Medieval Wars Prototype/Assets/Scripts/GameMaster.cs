...
public class GameMaster : MonoBehaviour
{
    ...
    // // public HandleUIButton handleUIButton;
    ...
    //             // // move and prepare the attack if possible.
    //             GridCell savedCell = unit.occupiedCell;

    //             MoveUnitAndPrePareAttack(unit, cell);

    //             // bool isThereIsAttackableUnit = unit.enemiesInRange.Any();
    //             // handleUIButton.cell = savedCell;
    //             // handleUIButton.unit = unit;

    //             // handleUIButton.ActivateButtons(isThereIsAttackableUnit);
    ...
    //             // then , we wait to click on one from the activated buttons.

    //             // click cancel => return back to the previous cell .
    //             // click attend => confirming move to the new cell .
    //             // click attack => choose one from the attackable unit and confirm attacking or return to the menu : attack - attend - cancel .


    //             // we have this possible actions :  attack , confirmMoving , cancelMoving , displayButtonsMenu
    ...
    // public void goBackToCell(GridCell previousCell, Unit unit)
    // {
    //     // move the unit to the previous cell
    //     Debug.Log("go back to the previous cell");
    //     unit.Move(previousCell.row, previousCell.column);
    //     //!!!1 here we can return back to the first cell the same way we leave it , 
    //     //!! wich means , we move first on Y then X .

    //     // when we move then click cancel , it's like we never move , 
    //     // exaclty the same state as the begining of a new turn ,
    //     unit.ResetUnitPropritiesInEndTurn();
    // }


    // public void MoveUnitAndPrePareAttack(Unit unit, GridCell cell)
    // {
